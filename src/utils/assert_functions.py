        ...
        for single_tuple in cfg["evaluate_with"]:
            for model in single_tuple:
                ...
                assert model[1] in students_cfg[model[0]]["languages"], \
                    "Language {} for Model {} in evaluation config is not defined in students_cfg".format(model[1], model[0])
...
def assert_embedding_cfg(students_cfg, embedding_sharing_cfg):
    ...
    if isinstance(embedding_sharing_cfg, str):
        assert embedding_sharing_cfg in options, \
            "Specified option '{}' in embed_sharing is not implemented. Please choose one of the following options: {}".format(embedding_sharing_cfg, options)
    else:
        ...
        for single_tuple in new_cfg:
            for model in single_tuple:
                ...
                assert model[1] in students_cfg[model[0]]["languages"], \
                    "Language {} for Model {} in embed_sharing is not defined in students_cfg".format(model[1], model[0])
...
def assert_weight_sharing_cfg(students_cfg, weight_sharing_cfg):
    ...
    if isinstance(weight_sharing_cfg, (str, bool)):
        assert weight_sharing_cfg in options, \
            "Specified option '{}' in embed_sharing is not implemented. Please choose one of the following options: {}".format(
                weight_sharing_cfg, options)
